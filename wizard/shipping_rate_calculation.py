...
class shipping_rate_wizard(orm.TransientModel):
    ...
    def update_shipping_cost(self, cr, uid, ids, context=None):
        ...
        ship_method_name = data.ups_service_id and data.ups_service_id.description or ''
        ship_method_ids = self.pool.get('shipping.rate.config').search(
            cr, uid, [('name','=',ship_method_name)], context=context
        )
...
#        if context.get('active_model', False) == 'sale.order':
        if context.get('active_model', False) in ['sale.order', 'account.invoice'] and 'active_id' in context:
            ...
            access_license_no = data.ups_shipper_id and  data.ups_shipper_id.access_license or ''
